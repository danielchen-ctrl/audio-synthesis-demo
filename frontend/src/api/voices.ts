...
export async function listVoicesFromDB(language?: string): Promise<Voice[]> {
  ...
  const resp = await client.get(`/api/v1/voices${params}`)
  return resp.data
}
...
export async function registerVoice(
  ...
  const resp = await client.post(`/api/v1/voices?${params}`, form, {
...
export async function deleteVoice(voiceId: string, deleteRemote = false): Promise<void> {
  await client.delete(`/api/v1/voices/${voiceId}?delete_remote=${deleteRemote}`)
}
